<template>
  <div id="outdoorRadiationComponent"></div>
</template>
  ...
  methods: {
    drawChart() {
      this.myChart2 = this.$echarts.init(
        document.getElementById("outdoorRadiationComponent")
      );

      var minTemp = Math.min(...this.outdoorRadiationData.data) - 0.1;
      var maxTemp = Math.max(...this.outdoorRadiationData.data) + 0.1;
      ...
      let option = {
        ...
        xAxis: {
          type: "category",
          data: this.outdoorRadiationData.time,
          //   splitNum: 4,
        ...
        series: [
          {
            data: this.outdoorRadiationData.data,
            showSymbol: false,
            type: "line",
            smooth: true,
          },
