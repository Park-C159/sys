<template>
  <div id="outdoorHumidityComponent"></div>
</template>
  ...
  methods: {
    drawChart() {
      this.myChart1 = this.$echarts.init(
        document.getElementById("outdoorHumidityComponent")
      );

      var minTemp = Math.min(...this.outdoorHumidityData.data) - 0.1;
      var maxTemp = Math.max(...this.outdoorHumidityData.data) + 0.1;
      ...
      let option = {
        ...
        xAxis: {
          type: "category",
          data: this.outdoorHumidityData.time,
          axisLabel: {
            showMaxLabel: true,
            showMinLabel: true,
        ...
        series: [
          {
            data: this.outdoorHumidityData.data,
            type: "bar",
            showBackground: true,
            backgroundStyle: {
