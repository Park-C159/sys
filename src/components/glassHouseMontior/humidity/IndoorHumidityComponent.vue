<template>
  <div id="indoorHumidityComponent"></div>
</template>
  ...
  methods: {
    drawChart() {
      this.myChart = this.$echarts.init(
        document.getElementById("indoorHumidityComponent")
      );

      var minTemp = Math.min(...this.indoorHumidityData.data) - 0.1;
      var maxTemp = Math.max(...this.indoorHumidityData.data) + 0.1;
      ...
      let option = {
        ...
        xAxis: {
          type: "category",
          data: this.indoorHumidityData.time,
          axisLabel: {
            showMaxLabel: true,
            showMinLabel: true,
        ...
        series: [
          {
            data: this.indoorHumidityData.data,
            type: "bar",
            showBackground: true,
            backgroundStyle: {
